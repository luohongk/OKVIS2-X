...
TASK_STATUSES = ("queued", "extracting", "vio", "succeeded", "failed", "interrupted")
TERMINAL_STATUSES = {"succeeded", "failed", "interrupted"}
ALLOWED_TRANSITIONS = {
    "queued": {"extracting", "failed", "interrupted"},
    "extracting": {"vio", "failed", "interrupted"},
    "vio": {"succeeded", "failed", "interrupted"},
    "succeeded": set(),
    "failed": set(),
    "interrupted": set(),
}
        ...
        with self.connect() as connection:
            ...
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS task_groups (
                    ...
                    config_rel TEXT NOT NULL,
                    config_sha256 TEXT NOT NULL,
                    cam_workers INTEGER NOT NULL CHECK (cam_workers BETWEEN 1 AND 4),
                    keep_images INTEGER NOT NULL CHECK (keep_images IN (0, 1)),
                ...
                CREATE TABLE IF NOT EXISTS tasks (
                    ...
                    status TEXT NOT NULL CHECK (
                        status IN ('queued', 'extracting', 'vio', 'succeeded', 'failed', 'interrupted')
                    ),
                    created_at TEXT NOT NULL,
                    started_at TEXT,
                    finished_at TEXT,
            ...
            with connection:
                connection.execute(
                    ...
                    INSERT INTO task_groups (
                        id, device, config_rel, config_sha256,
                        cam_workers, keep_images, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        group["id"],
                ...
                for task in task_records:
                    connection.execute(
                        """
                        INSERT INTO tasks (
                            id, group_id, sequence, status, created_at, task_root,
                            started_at, finished_at, exit_code, runner_status, error_summary
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """,
                        (
                            task["id"],
    ...
    def get_task_details(self, task_id: str) -> dict[str, Any] | None:
        with self.connect() as connection:
            row = connection.execute(
                """
                SELECT t.*, g.device, g.config_rel, g.config_sha256,
                       g.cam_workers, g.keep_images
                FROM tasks AS t
            ...
            with connection:
                ...
                if new_status not in ALLOWED_TRANSITIONS[current_status]:
                    raise InvalidStatusTransition(
                        f"cannot transition task {task_id} from {current_status} to {new_status}"
                    )
                assignments = ["status = ?"]
                ...
                for name, value in fields.items():
                    ...
                    values.append(value)
                values.extend((task_id, current_status))
                cursor = connection.execute(
                    f"UPDATE tasks SET {', '.join(assignments)} WHERE id = ? AND status = ?",
                    values,
                )
                if cursor.rowcount != 1:
                    raise InvalidStatusTransition("task status changed concurrently")
    ...
    def list_tasks(
        ...
        *,
        ...
        with self.connect() as connection:
            rows = connection.execute(
                f"""
                SELECT t.*, g.device, g.config_rel, g.cam_workers, g.keep_images
                FROM tasks AS t
                JOIN task_groups AS g ON g.id = t.group_id
                {where}
        ...
        with self.connect() as connection:
            with connection:
                cursor = connection.execute(
                    ...
                    WHERE status IN ('queued', 'extracting', 'vio')
                    """,
                    (finished_at,),
                )
